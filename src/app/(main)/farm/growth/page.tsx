  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <KpiCard label="Mean canopy/plant" value={farm.kpis.growth.meanCanopyPerPlant} hint="estate-wide" accent="olive" />
        <KpiCard label="Median canopy/plant" value={farm.kpis.growth.medianCanopyPerPlant} hint="estate-wide" accent="sage" />
      </div>
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <AreaChart data={farm.canopy}>
                ...
                <Tooltip contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 12 }} formatter={(value) => [`${value}%`, "Canopy cover"] as [string, string]} />
                <Area type="monotone" dataKey="pct" stroke="var(--olive)" strokeWidth={2} fill="url(#cg)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <BarChart data={farm.variance}>
                <CartesianGrid stroke="var(--border)" strokeDasharray="3 3" vertical={false} />
                <XAxis dataKey="sector" stroke="var(--muted-foreground)" fontSize={11} tickLine={false} axisLine={false} />
                <YAxis stroke="var(--muted-foreground)" fontSize={11} tickLine={false} axisLine={false} />
                <Tooltip contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 12 }} />
                <Bar dataKey="cv" fill="var(--clay)" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
